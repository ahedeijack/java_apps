    ...
    public function make_request_params(array $args)
    {
        ...
        $this->add_payment_params
                (
                    array
                    (                       
                        'SHIPTOSTREET'      => ( WC()->version < '2.7.0' ) ? WC()->customer->get_address() : WC()->customer->get_billing_address(),
                        'SHIPTOSTREET2'     => ( WC()->version < '2.7.0' ) ? WC()->customer->get_address_2() : WC()->customer->get_billing_address_2(),
                        'SHIPTOCITY'        => ( WC()->version < '2.7.0' ) ? WC()->customer->get_city() : WC()->customer->get_billing_city(),
                        'SHIPTOSTATE'       => ( WC()->version < '2.7.0' ) ? WC()->customer->get_state() : WC()->customer->get_billing_state(),
                        'SHIPTOZIP'         => ( WC()->version < '2.7.0' ) ? WC()->customer->get_postcode() : WC()->customer->get_billing_postcode(),
                        'SHIPTOCOUNTRYCODE' => ( WC()->version < '2.7.0' ) ? WC()->customer->get_country() : WC()->customer->get_billing_country(),
                        'SHIPTOPHONENUM'    =>  WC()->session->post_data['billing_phone'],
                        'NOTETEXT'          =>  WC()->session->post_data['order_comments'],
                        'EMAIL'             => ( WC()->version < '2.7.0' ) ? WC()->session->post_data['billing_email'] : WC()->customer->get_billing_email(),
                    )
                );
